...
async function startServer() {
  ...
  // API Route: Synthesis Hub (Background Process Capable)
  app.post("/api/ai/synthesize", async (req, res) => {
    const { generationId, userId, preferences, readyVideos } = req.body;
    
    if (!generationId || !userId || !preferences) {
      return res.status(400).json({ error: "Missing required synthesis parameters" });
    }

    // Immediately respond to the client so they know it's queued/processing
    // but we will continue the process in the background.
    res.json({ status: "processing", message: "Synthesis protocol initialized in background." });

    const db = getDb();
    const genRef = db.collection("generations").doc(generationId);

    try {
      console.log(`[SYNTHESIS] Starting background job for ${generationId} (User: ${userId})`);
      
      const isOpenRouter = preferences.model.includes("/");
      let generatedContent = "";

      if (isOpenRouter) {
        // Fetch API Key from Firestore if not provided (safer)
        const keyDoc = await db.collection("users").doc(userId).collection("private").doc("keys").get();
        const apiKey = keyDoc.exists ? keyDoc.data()?.openRouterKey : null;
        
        if (!apiKey) throw new Error("OpenRouter API Key missing in synthesis protocol.");
...
${readyVideos.map((v: any) => `- ID: ${v.firestoreId || v.id} | Name: ${v.name}`).join('\n')}

Synthesize the content from these assets into a cohesive narrative. Do not just list them.`;

        let additionalText = "";
        readyVideos.forEach((v: any) => { if (v.extractedText) additionalText += `\n\n[Content ${v.name}]:\n${v.extractedText}`; });
        ...
        const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
          ...
          headers: {
            "Authorization": `Bearer ${apiKey}`,
            "Content-Type": "application/json",
            "X-Title": "Velocity Background Synthesis"
          },
          body: JSON.stringify({
        ...
        const data = await response.json() as any;
        if (!response.ok) throw new Error(data.error?.message || "OpenRouter Background Synthesis Failed");
        generatedContent = data.choices?.[0]?.message?.content;
      } else {
        // Gemini Logic
        if (!process.env.GEMINI_API_KEY) throw new Error("GEMINI_API_KEY is not configured on server.");
        const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });

        const prompt = `${preferences.systemPrompt}\nTransform provided media into a high-quality, professional blog post.

CRITICAL ARCHITECTURE REQUIREMENTS:
1. H1_TITLE: ALWAYS start with a compelling H1 title.
2. STRUCTURAL_HIERARCHY: Use H2/H3 markers.
3. NARRATIVE_QUOTES: Include at least TWO blockquotes.
4. COHESIVE_FLOW: Comprehensive arc.
...
Synthesize accurately.`;
        ...
        // For simplicity in the background, we rely mainly on the extracted text for now 
        // to avoid re-fetching large blobs on the server unless strictly necessary.
        // If the user needs full multimodal background synthesis, we would need to fetch the storageUrls here.
        const response = await ai.models.generateContent({
          model: preferences.model || "gemini-3-flash-preview",
          contents: prompt + additionalText
        });
        
        generatedContent = response.text || "";
      }

      if (!generatedContent) throw new Error("AI Synthesis returned null payload.");

      await genRef.update({
        content: generatedContent,
        title: generatedContent.split('\n')[0].replace(/^#+\s*/, '').substring(0, 100) || "Untitled Synthesis",
        status: "completed",
        updatedAt: admin.firestore.FieldValue.serverTimestamp()
      });

      console.log(`[SYNTHESIS] Successfully finalized background job: ${generationId}`);

    } catch (err: any) {
      console.error(`[SYNTHESIS] Background job failed for ${generationId}:`, err);
      await genRef.update({
        status: "failed",
        error: err.message || "Unknown background process error",
        updatedAt: admin.firestore.FieldValue.serverTimestamp()
      }).catch(e => console.error("Could not update failure status:", e));
    }
  });
