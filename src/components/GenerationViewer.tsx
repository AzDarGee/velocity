...
import { X, Download, Copy, Check, Type, FileAudio, FileText, FileCode, FileType, Columns } from "lucide-react";
...
export function GenerationViewer({ content, title, theme, isAdmin, mediaFiles, onClose, onDownload, onCopy, onDownloadAsset }: GenerationViewerProps) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: `prose ${theme === 'dark' ? 'prose-invert' : 'prose-neutral'} prose-headings:font-serif prose-headings:italic prose-p:font-sans prose-p:leading-relaxed selection:bg-yellow-200 selection:text-black focus:outline-none w-full max-w-none`,
      },
    },
  });
      ...
      <main className="flex-1 overflow-y-auto history-scrollbar p-8 md:p-20 flex justify-center">
        <article className="max-w-3xl w-full">
          ...
          {activeMode === 'rich-text' && (
            <div className={`p-4 min-h-[500px] border-2 ${theme === 'dark' ? 'border-[#333] bg-[#111]' : 'border-[#141414] bg-white'}`}>
              <EditorContent editor={editor} />
            </div>
          )}
          {activeMode === 'html' && (
            <div className={`p-4 min-h-[500px] border-2 font-mono text-sm whitespace-pre-wrap ${theme === 'dark' ? 'border-[#333] bg-[#111] text-indigo-300' : 'border-[#141414] bg-white text-indigo-700'}`}>
